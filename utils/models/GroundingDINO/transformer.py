...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(
        ...
        self.norm1 = nn.LayerNorm(d_model,dtype=torch.half)

        # cross attention text
        if use_text_cross_attention:
            self.ca_text = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
            self.catext_dropout = nn.Dropout(dropout) if dropout > 0 else nn.Identity()
            self.catext_norm = nn.LayerNorm(d_model,dtype=torch.half)

        # self attention
        self.self_attn = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.dropout2 = nn.Dropout(dropout) if dropout > 0 else nn.Identity()
        self.norm2 = nn.LayerNorm(d_model,dtype=torch.half)
        ...
        self.norm3 = nn.LayerNorm(d_model,dtype=torch.half)
    ...
    def forward_ffn(self, tgt):
        with torch.cuda.amp.autocast(enabled=False):
            # inter = self.linear1(tgt)
            # inter = self.activation(inter)
            # inter = self.dropout3(inter)
            # tgt2 = self.linear2(inter)
            # print(tgt.dtype)
            # print(self.linear1.weight.dtype)
            tgt = tgt.to(self.linear1.weight.dtype)
