...
from .mixin import LlamaVisionExpertFCMixin, LlamaVisionExpertAttnMixin, GroundingMixin
...
class CogAgentModelNew(LLaMAModel):
    def __init__(self, args, transformer=None, parallel_output=True, **kwargs):
        ...
        self.add_mixin("grounding", GroundingMixin(load_vision_branch_args()))
        # Projection layer
        grounding_fc = [
            nn.Linear(args.hidden_size, args.hidden_size),
            nn.ReLU(inplace=True),
            nn.Linear(args.hidden_size, 256),#out_dim),
            nn.Dropout(0.0),
        ]
        self.grounding_fc = nn.Sequential(*grounding_fc)
        self.vg_token_idx =  args.vg_token_idx # [VG] id
    ...
    def forward(self, input_ids, vision_expert_mask, image_embed_mask, **kwargs):
        ...
        if input_ids.shape[1] > 1:
            llm_output = super().forward(input_ids=input_ids, vision_expert_mask=vision_expert_mask, image_embed_mask=image_embed_mask, **kwargs)
        else:
            llm_output = super().forward(input_ids=input_ids, **kwargs)
        ...
        for i in range(len(pred_embeddings)):  # process each data
            gt_ids.append(i)
            bboxes = kwargs['bboxes_gt_list'][i]
            target.append({
...
class FineTuneTestCogAgentModelNew(CogAgentModelNew):
    def __init__(self, args, transformer=None, parallel_output=True, **kw_args):
        super().__init__(args, transformer=transformer, parallel_output=parallel_output, **kw_args)
        if args.use_ptuning:
