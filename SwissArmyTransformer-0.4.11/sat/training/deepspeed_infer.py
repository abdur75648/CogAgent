...
def infer_main(args, model_cls, forward_step_function, create_dataset_function, handle_metrics_function=None, init_function=None, collate_fn=None, forward_step_eval=None):
    ...
    if args.load is not None:
        print_rank0('Loading model training checkpoint from {}'.format(args.load))
        ...
        raise ValueError("Can't do inference without loading a model!")
    # if args.save:
    ...
    inference_results = inference_function(model, test_data, args, hooks) 
...
def inference_function(model,
                       ...
                       hooks={}):
    """Run Inference"""
    ######### Here we don't need to train the model #########
    ...
    results = infer_step(test_data_iterator,
                                           model,
                                           args, hooks)
...
def infer_step(data_iterator, model, args, hooks={}):
    ...
    results = forward_step(data_iterator, model, args)
    return results
